...
export const OtpInputComponent = ({
    maxLength,
    onOtpChangeFunction,
    otpEnterCompleteFunction,
    className,
}: {
    maxLength: number,
    className?: string,
    onOtpChangeFunction: Dispatch<SetStateAction<string>>;
    otpEnterCompleteFunction: Dispatch<SetStateAction<boolean>>;
}) => {
    return (
        <OTPInput
            onChange={(value) => {
                if (value.length === maxLength) {
                    otpEnterCompleteFunction(true);
                }
                onOtpChangeFunction(value)
            }}
            maxLength={maxLength}
